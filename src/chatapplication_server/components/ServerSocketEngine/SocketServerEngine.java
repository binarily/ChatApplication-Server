...
public class SocketServerEngine extends GenericThreadedComponent
{
    ...
    public SocketServerEngine() 
    {
        ...
        try 
        {
            AccessCerts key_store = new AccessCerts("Server");
            this.name = key_store.getUser();
    ...
    public void writeMsgSpecificClient( int PortNo, String msg )
    {
        ...
        for ( int i = 0; i < occupance.size(); i++ )
        {
            ...
            /** If this is the correct client... get his public key and encrypt the message with it */

            if ( sch.getHandleSocket().getPort() == PortNo )
            {
                PublicKey receipient_key = all_certificates.get(sch.getUserName()).getPublicKey();
                ...
                {
                    SocketServerGUI.getInstance().appendEvent("Encryption failed: " + e.getMessage() + "\n");
                    ComponentManager.getInstance().fatalException(e);
                }
            }
        }
